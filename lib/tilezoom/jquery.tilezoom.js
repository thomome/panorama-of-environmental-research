...
var methods = {
	...
	zoom : function( level, coords, instant) {
		return this.each(function(){
			var $cont = $(this);
			var settings = $cont.data('tilezoom.settings');
			var speed = instant ? 1 : settings.speed;
	...
	reposition : function( level, coords, speed ) {
		return this.each(function(){
			var $cont = $(this);
			var settings = $cont.data('tilezoom.settings');
			if(settings.inAction) return false;
			...
			if(9 <= level && level < settings.numLevels) {
				...
				$cont.data('tilezoom.settings', settings);
				initTiles($cont);
				setSizePosition($cont, {left: 0, top: 0}, speed, function() {
...
function initDraggable($cont) {
	...
	$holder.off('mousedown touchstart');
	$holder.off('mousemove touchmove');
	...
	$holder.on('mousedown touchstart', function(e) {
		...
		startX = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0].pageX : e.pageX;
		startY = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0].pageY : e.pageY;
		pos = {};
	...
	$(document).on('mousemove touchmove', function(e) {
		var compX = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0].pageX : e.pageX;
		var compY = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0].pageY : e.pageY;
		if(e.originalEvent.changedTouches && e.originalEvent.changedTouches.length > 1) dragging = false
		if(dragging==true){
			if(downEvent) downEvent.preventDefault();
			var offsetX =  compX - startX;
			var offsetY =  compY - startY;
...
function initMousewheel($cont) {
	...
	var hammertime = new Hammer($cont[0]);
	...
	hammertime.on('pinch', function(ev) {
		...
		var scale = (ev.scale-1)*0.3+1
		$cont.tilezoom('zoom', Math.round(scale*startLevel), coords, true);
	});

	$cont.unbind('dblclick');
	$cont.dblclick(function(e){
		var coords = {};
		if(settings.zoomToCursor) {
			coords.x = e.pageX;
			coords.y = e.pageY;
